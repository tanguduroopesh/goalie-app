...
def generate_tasks_for_goal(goal_title, days=7):
    ...
    # If user entered comma-separated tasks
    if "," in base or ";" in base:
        ...
        while len(tasks) < days:
            ...
            tasks.append({
                "title": f"Work on: {base} — step {i + 1}",
                "due_date": due
            })

        return tasks[:days]

    # Generic daily breakdown
    for i in range(days):

        due = datetime.utcnow().date() + timedelta(days=i)

        tasks.append({
            "title": f"Day {i + 1}: Work on — {base}",
            "due_date": due
        })
...
# ---------- Streamlit UI ----------
st.set_page_config(
    page_title="Goal/Action Planner",
    layout="wide"
)
...
if not st.session_state.user_id:
    ...
    st.write(
        "This prototype demonstrates goal creation, automatic "
        "task breakdown, and progress tracking."
    )

    st.info(
        "Register or login from the sidebar "
        "(demo: click Init demo DB, then login with demo/demo)."
    )
...
with col2:
    ...
    with st.form("create_goal"):
        ...
        days = st.number_input(
            "Number of daily tasks to create",
            min_value=1,
            max_value=30,
            value=7
        )
...
st.caption(
    "Prototype: simple auth and task generation. "
    "For production, add migrations, stronger security, "
